...
class Codemaps :
    ...
    def __create_indexs(self, data, maxlen, suflen) :
        ...
        for s in data.sentences() :
            for t in s :
                ...
                sufs2.add(t['lc_form'][-self.suflen2:])
                labels.add(t['tag'])
        ...
        self.suf_index2 = {s: i+2 for i,s in enumerate(list(sufs2))}
        self.suf_index2['PAD'] = 0  # Padding
        self.suf_index2['UNK'] = 1  # Unknown suffixes
    ...
    def __load(self, name) : 
        ...
        with open(name+".idx") as f :
            for line in f.readlines(): 
                ...
                elif t == 'SUFLEN2' : self.suflen2 = int(k)                
                elif t == 'WORD': self.word_index[k] = int(i)
                elif t == 'SUF': self.suf_index[k] = int(i)
                elif t == 'SUF2': self.suf_index2[k] = int(i)
    ...
    def save(self, name) :
        ...
        with open(name+".idx","w") as f :
            ...
            print ('SUFLEN2', self.suflen2, "-", file=f)
            ...
            for key in self.suf_index2 : print('SUF2', key, self.suf_index2[key], file=f)
##############################################################################################
...
##############################################################
    ...
    def encode_words(self, data):
        ...
        enc_sufs2 = [
            torch.Tensor([
                self.suf_index2[w['lc_form'][-self.suflen2:]] if w['lc_form'][-self.suflen2:] in self.suf_index2 else self.suf_index2['UNK']
                for w in s
            ]) for s in data.sentences()
        ]
        # truncate sentences longer than maxlen
        enc_sufs2 = [s[0:self.maxlen] for s in enc_sufs]
        # create a tensor full of padding
        Xs2 = torch.full((len(enc_sufs2), self.maxlen), self.suf_index2['PAD'], dtype=torch.int64)
        for i, s in enumerate(enc_sufs2):
            Xs2[i, 0:len(s)] = s
        ...
        return [Xw, Xs, Xs2, Xl, Xcap, Xdash, Xnum, Xext, Xspecial, Xlen, Xpos]
    ...
    ## -------- get suf2 index size ---------
    def get_n_sufs2(self) :
        return len(self.suf_index2)
    ## -------- get suf index size ---------
    def get_n_sufs(self) :
        return len(self.suf_index)
